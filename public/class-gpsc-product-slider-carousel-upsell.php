...
class GPSC_Product_Slider_Carousel_Upsell {
	...
	function gpsc_add_custom_upsell_product_display() {
		// Get options.
		$wpgpsc_upsell_slider_show            = wpgpsc_get_options( 'wpgpsc_upsell_slider_show' );
		$wpgpsc_upsell_section_title_show     = wpgpsc_get_options( 'wpgpsc_upsell_section_title_show' );
		$wpgpsc_upsell_section_title_text     = wpgpsc_get_options( 'wpgpsc_upsell_section_title_text' );
		$wpgpsc_upsell_slider_speed           = wpgpsc_get_options( 'wpgpsc_upsell_slider_speed' );
		$wpgpsc_upsell_slider_autoplay_on     = wpgpsc_get_options( 'wpgpsc_upsell_slider_autoplay' );
		$wpgpsc_upsell_slider_autoplay_delay  = wpgpsc_get_options( 'wpgpsc_upsell_slider_autoplay_delay' );
		$wpgpsc_upsell_slider_autoplay        = $wpgpsc_upsell_slider_autoplay_on ? 'autoplay: {delay: ' . $wpgpsc_upsell_slider_autoplay_delay . ',},' : '';
		$wpgpsc_upsell_slider_loop            = wpgpsc_get_options( 'wpgpsc_upsell_slider_loop' ) ? 'true' : 'false';
		$wpgpsc_upsell_slides_per_view        = wpgpsc_get_options( 'wpgpsc_upsell_slides_per_view' );
		$wpgpsc_upsell_slides_space_between   = wpgpsc_get_options( 'wpgpsc_upsell_slides_space_between' );
		$wpgpsc_upsell_slider_navigation      = wpgpsc_get_options( 'wpgpsc_upsell_slider_navigation' );
		$wpgpsc_upsell_slider_nav_icon        = wpgpsc_get_options( 'wpgpsc_upsell_slider_nav_icon' );
		$wpgpsc_upsell_slider_pagination      = wpgpsc_get_options( 'wpgpsc_upsell_slider_pagination' );
		$wpgpsc_upsell_slider_pagination_type = wpgpsc_get_options( 'wpgpsc_upsell_slider_pagination_type' );

		// Visibilities.
		$wpgpsc_upsell_section_title_show   = wpgpsc_get_options( 'wpgpsc_upsell_section_title_show' );
		$wpgpsc_upsell_section_title_text   = wpgpsc_get_options( 'wpgpsc_upsell_section_title_text' );
		$wpgpsc_upsell_product_img_show     = wpgpsc_get_options( 'wpgpsc_upsell_product_img_show' );
		$wpgpsc_upsell_product_name_show    = wpgpsc_get_options( 'wpgpsc_upsell_product_name_show' );
		$wpgpsc_upsell_product_excerpt_show = wpgpsc_get_options( 'wpgpsc_upsell_product_excerpt_show' );
		$wpgpsc_upsell_product_cat_show     = wpgpsc_get_options( 'wpgpsc_upsell_product_cat_show' );
		$wpgpsc_upsell_product_rating_show  = wpgpsc_get_options( 'wpgpsc_upsell_product_rating_show' );
		$wpgpsc_upsell_product_stack_show   = wpgpsc_get_options( 'wpgpsc_upsell_product_stack_show' );
		$wpgpsc_upsell_product_price_show   = wpgpsc_get_options( 'wpgpsc_upsell_product_price_show' );
		$wpgpsc_upsell_product_btn_show     = wpgpsc_get_options( 'wpgpsc_upsell_product_btn_show' );
		 ...
		 * Nav Icon Generate.
		 */
		$wpgpsc_upsell_slider_nav_svg = '';
		switch ( $wpgpsc_upsell_slider_nav_icon ) {
			case 'ico-1':
				$wpgpsc_upsell_slider_nav_svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 512"><path d="M224.3 273l-136 136c-9.4 9.4-24.6 9.4-33.9 0l-22.6-22.6c-9.4-9.4-9.4-24.6 0-33.9l96.4-96.4-96.4-96.4c-9.4-9.4-9.4-24.6 0-33.9L54.3 103c9.4-9.4 24.6-9.4 33.9 0l136 136c9.5 9.4 9.5 24.6.1 34z"/></svg>';
				break;
			case 'ico-2':
				...
				break;
			case 'ico-3':
				$wpgpsc_upsell_slider_nav_svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 192 512"><path d="M0 384.662V127.338c0-17.818 21.543-26.741 34.142-14.142l128.662 128.662c7.81 7.81 7.81 20.474 0 28.284L34.142 398.804C21.543 411.404 0 402.48 0 384.662z"/></svg>';
				break;
			case 'ico-4':
		...
		if ( ! empty( $gpsc_upsell_product_ids ) ) {
			...
			// Get upsell products query.
			$gpsc_upsell_args     = array(
				'include'    => $gpsc_upsell_product_ids,
				'status'     => 'publish',
				'visibility' => 'visible',
			);
			$gpsc_upsell_products = wc_get_products( $gpsc_upsell_args );
			...
			if ( $wpgpsc_upsell_slider_navigation ) {
				echo '.gpsc-slider-btn-next::after,
			...
			if ( $wpgpsc_upsell_slider_pagination ) {
				echo '.gpsc-upsell-product-pagination .swiper-pagination-bullet-active {
				...
				.gpsc-upsell-product-pagination {
					bottom: 0 !important;
					top: auto !important;
				}
				.gpsc-upsell-product-carousel {
			...
			if ( 'progressbar' == $wpgpsc_upsell_slider_pagination_type ) {
				echo '.gpsc-upsell-product-carousel {
			...
			echo '<div class="swiper gpsc-upsell-product-carousel">';
			if ( $wpgpsc_upsell_section_title_show ) {

				echo '<h4 class="gpsc-upsell-slider-title">' . $wpgpsc_upsell_section_title_text . '</h4>';
			}
			if ( $wpgpsc_upsell_slider_navigation ) {
				echo '<div class="gpsc-slider-btn">
				<div class="gpsc-slider-btn-prev swiper-button-prev">' . $wpgpsc_upsell_slider_nav_svg . '</div>
				<div class="gpsc-slider-btn-next swiper-button-next">' . $wpgpsc_upsell_slider_nav_svg . '</div>
			</div>';
			}
			echo '<div class="swiper-wrapper gpsc-upsell-main--wrapper">';
			foreach ( $gpsc_upsell_products as $product ) {
				...
				echo '<div class="swiper-slide gpsc-upsell-swiper-slide">
						<div class="gpsc-upsell-single-slide">';
				if ( $wpgpsc_upsell_product_img_show && get_post_thumbnail_id($product->get_id()) ) {
					echo '<div class="gpsc-upsell-product-img"><a href="' . esc_url( get_permalink( $product->get_id() ) ) . '">
				<img src="' . esc_url( $gpsc_product_img_src ) . '" alt="' . esc_html( $product->get_title() ) . '">
				</a>
					</div>';
				}
				echo '<div class="gpsc-upsell-product-content-wrapper">
				<div class="gpsc-upsell-product-content">';
				if ( $wpgpsc_upsell_product_name_show ) {
					echo '<div class="gpsc-upsell-product-name"><a href="' . esc_url( get_permalink( $product->get_id() ) ) . '">' . $product->get_title() . '</a></div>';
				}
				if ( $wpgpsc_upsell_product_excerpt_show ) {
					echo '<div class="gpsc-upsell-product-desc">';
					echo wp_trim_words( wp_kses_post( wpautop( $product->get_short_description() ) ), 6, ' […]' );
					echo '</div>';
				}
				if ( $wpgpsc_upsell_product_cat_show ) {
					echo '<div class="gpsc-upsell-product-category">';
				...
				echo '</div>
				<div class="gpsc-upsell-pro-stock-rating">';
				if ( $product->get_reviews_allowed() && $wpgpsc_upsell_product_rating_show ) {
					$gpsc_product_rating_percentage = (int) $product->get_average_rating() * 2 . '0%';
					echo '<div class="gpsc-upsell-product-rating-wrapper"><div class="gpsc-upsell-product-star-rating" role="img" aria-label="Rated ' . $product->get_average_rating() . ' out of 5" style="width:' . $gpsc_product_rating_percentage . '"><span>Rated <strong class="rating">4.00</strong> out of 5</span></div></div>';
				}
				if ( $wpgpsc_upsell_product_stack_show ) {
					echo '<div class="gpsc-upsell-product-stock">';
					switch ( $product->get_stock_status() ) {
						case 'instock':
							echo '<div class="gpsc-upsell-product-stock-content"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" style="height: 16px; width: 16px;"><path fill="#238527" d="M470.6 105.4c12.5 12.5 12.5 32.8 0 45.3l-256 256c-12.5 12.5-32.8 12.5-45.3 0l-128-128c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0L192 338.7 425.4 105.4c12.5-12.5 32.8-12.5 45.3 0z"/></svg><span>In stock</span></div>';
							break;
						case 'outofstock':
							echo '<div class="gpsc-upsell-product-stock-content"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><path fill="#ff0000" d="M310.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L160 210.7 54.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L114.7 256 9.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L160 301.3 265.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L205.3 256 310.6 150.6z"/></svg><span>Out of stock</span></div>';
							break;
						case 'onbackorder':
							echo '<div class="gpsc-upsell-product-stock-content"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" style="height: 14px;width: 14px;"><path fill="#0170b9" d="M109.7 160H160c17.7 0 32 14.3 32 32s-14.3 32-32 32H32c-17.7 0-32-14.3-32-32V64C0 46.3 14.3 32 32 32s32 14.3 32 32v51.2L81.6 97.6c87.5-87.5 229.3-87.5 316.8 0s87.5 229.3 0 316.8s-229.3 87.5-316.8 0c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0c62.5 62.5 163.8 62.5 226.3 0s62.5-163.8 0-226.3s-163.8-62.5-226.3 0L109.7 160z"/></svg><span>On backorder</span></div>';
							break;
					}
					echo '</div>';
				}
				if ( $wpgpsc_upsell_product_price_show ) {
					echo '<div class="gpsc-upsell-product-price-wrapper"><bdi class="gpsc-single-upsell-product-price">' . wp_kses_post( $product->get_price_html() ) . '</bdi></div>';
				}
				echo '</div>';
				if ( $wpgpsc_upsell_product_btn_show ) {
					echo '<div class="gpsc-upsell-product-btn">';
			...
			echo '</div>';
			if ( $wpgpsc_upsell_slider_pagination ) {
				echo '<div class="gpsc-upsell-product-pagination swiper-pagination"></div>';
			}
			echo '</div>';
...
(function( $ ) {
	...
	$( document ).ready(function() {
		var swiper = new Swiper(".gpsc-upsell-product-carousel", {
			' . $wpgpsc_upsell_slider_autoplay . '
			speed: ' . $wpgpsc_upsell_slider_speed . ',
			loop: ' . $wpgpsc_upsell_slider_loop . ',
			slidesPerView: 1,
			spaceBetween: ' . $wpgpsc_upsell_slides_space_between . ',
			breakpoints: {
				667: {
					slidesPerView: 2,
				},
				982: {
					slidesPerView: ' . $wpgpsc_upsell_slides_per_view . ',
					spaceBetween: ' . $wpgpsc_upsell_slides_space_between . ',
				}
			},
			pagination: {
				...
				clickable: true,
				dynamicBullets: true,
				type: "' . $wpgpsc_upsell_slider_pagination_type . '",
			},
			navigation: {
