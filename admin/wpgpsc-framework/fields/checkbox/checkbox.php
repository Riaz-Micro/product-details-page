    ...
    public function render() {
      ...
      if ( isset( $this->field['options'] ) ) {
        ...
        if ( is_array( $options ) && ! empty( $options ) ) {
          ...
          if ( $args['check_all'] ) {
            echo '<div class="wpgpsc-checkbox-all">'. esc_html( $args['check_all_text'] ) .'</div>';
          }
        ...
        echo '<input type="hidden" name="'. esc_attr( $this->field_name() ) .'" value="'. $this->value .'" class="wpgpsc--input"'. $this->field_attributes() .'/>';
        echo '<input type="checkbox" name="_pseudo" class="wpgpsc--checkbox"'. esc_attr( checked( $this->value, 1, false ) ) . $this->field_attributes() .'/>';
