 ...
 *
 */
if ( ! class_exists( 'WPGPSC_Field_link_color' ) ) {
  class WPGPSC_Field_link_color extends WPGPSC_Fields {
    ...
    public function render() {
      ...
      $color_props = array(
        'color'    => esc_html__( 'Normal', 'wpgpsc' ),
        'hover'    => esc_html__( 'Hover', 'wpgpsc' ),
        'active'   => esc_html__( 'Active', 'wpgpsc' ),
        'visited'  => esc_html__( 'Visited', 'wpgpsc' ),
        'focus'    => esc_html__( 'Focus', 'wpgpsc' )
      );
      ...
      foreach ( $color_props as $color_prop_key => $color_prop_value ) {
        ...
        if ( ! empty( $args[$color_prop_key] ) ) {
          ...
          echo '<div class="wpgpsc--title">'. esc_attr( $color_prop_value ) .'</div>';
          echo '<input type="text" name="'. esc_attr( $this->field_name( '['. $color_prop_key .']' ) ) .'" value="'. esc_attr( $value[$color_prop_key] ) .'" class="wpgpsc-color"'. $default_attr . $this->field_attributes() .'/>';
          echo '</div>';
