    ...
    public function render() {
      ...
      if ( ! empty( $options ) ) {
        foreach ( $options as $key => $option ) {
          ...
          echo '<div class="wpgpsc--title">'. $option .'</div>';
          echo '<input type="text" name="'. esc_attr( $this->field_name( '['. $key .']' ) ) .'" value="'. esc_attr( $color_value ) .'" class="wpgpsc-color"'. $default_attr . $this->field_attributes() .'/>';
          echo '</div>';
