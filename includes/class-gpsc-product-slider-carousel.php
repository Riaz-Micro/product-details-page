...
class GPSC_Product_Slider_Carousel {
	...
	public function __construct() {
		if ( defined( 'GPSC_PLUG_VERSION' ) ) {
			$this->version = GPSC_PLUG_VERSION;
		} else {
			$this->version = '1.0.0';
		}
	...
	private function load_dependencies() {
		...
		/**
		 * Allowed SVG code,
		 * Through kses extended rule set.
		 */
		function gpsc_get_kses_extended_ruleset() {
	...
	private function define_admin_hooks() {
		...
		// CPT Column.
		$this->loader->add_filter( 'manage_gpsc_slider_carousel_posts_columns', $plugin_admin_cpt, 'gpsc_admin_column' );
		$this->loader->add_action( 'manage_gpsc_slider_carousel_posts_custom_column', $plugin_admin_cpt, 'gpsc_admin_field', 10, 2 );
	}
	...
	private function define_public_hooks() {
		...
		if ( wpgpsc_get_options( 'gpsc_woo_templ_activated' ) ) {

			$this->loader->add_filter( 'template_include', $plugin_public, 'load_custom_single_product_template', 99 );
		}
		...
		if ( wpgpsc_get_options( 'wpgpsc_related_slider_show' ) ) {
		...
		if ( wpgpsc_get_options( 'wpgpsc_upsell_slider_show' ) ) {

			$this->loader->add_action( 'init', $plugin_public_upsell, 'gpsc_remove_default_upsell_product_display', 10 );

			$wpgpsc_related_products_position = 15;
			if ( wpgpsc_get_options( 'wpgpsc_related_before_upsells' ) ) {

				$wpgpsc_related_products_position = 25;
			}
			$this->loader->add_action( 'woocommerce_after_single_product_summary', $plugin_public_upsell, 'gpsc_add_custom_upsell_product_display', $wpgpsc_related_products_position );
		}

		$plugin_public_gallery = new GPSC_Product_Slider_Carousel_Gallery( $this->get_plugin_name(), $this->get_version() );
		if ( wpgpsc_get_options( 'wpgpsc_gallery_slider_show' ) ) {
		...
		if ( wpgpsc_get_options( 'wpgpsc_crossell_slider_show' ) ) {
